...
/**
 * Set default report access (DA-public, tid 1229) on oda_page nodes.
 */
function oda_deploy_10001_report_access(): void {
  $default_tid = 1229;
  ...
  $nids = $node_storage->getQuery()
    ->accessCheck(FALSE)
    ->condition('type', 'oda_page')
    ->notExists('field_oda_report_access')
    ->execute();

  foreach ($nids as $nid) {
    $node = $node_storage->load($nid);
    if ($node && $node->get('field_oda_report_access')->isEmpty()) {
      $node->set('field_oda_report_access', ['target_id' => $default_tid]);
      $node->save();
    }
  }
}
